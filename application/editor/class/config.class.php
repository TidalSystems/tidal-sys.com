...
class SpawConfig
{
 ...
 static function &configVar()
   ...
   * @static
   */
  static function getStaticConfigItem($name)
  {
    $cfg = &SpawConfig::configVar();
    if (isset($cfg[$name]))
      return $cfg[$name];
  ...
  function storeSecureConfig()
  {
    ...
    $sec_cfg = array();
    $result = '';
    $stored_cfg = (array)$this->SpawVars->getSessionVar("spaw_configs");
    ...
    if ($strcfg != '')
    {
      ...
      $this->SpawVars->setSessionVar("spaw_configs", $stored_cfg);
    }
    return $result;
  }
  ...
  {
    $sec_cfg = $this->SpawVars->getSessionVar("spaw_configs");
