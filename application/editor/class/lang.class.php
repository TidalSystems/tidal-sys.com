...
class SpawLang
{
  ...
  {
	  $this->SpawConfig= new SpawConfig();
    if ($lang == '')
    {
      // default lang
      $this->lang = $this->SpawConfig->getStaticConfigValue("default_lang");
    }
    else
    {
      $this->lang = $lang;
  ...
  {
    $spaw_root = $this->SpawConfig->getStaticConfigValue("SPAW_ROOT");
  ...
  function getMessage($message, $block='', $module='')
  {
    ...
    if ($msg != '')
    {
      if ($this->SpawConfig->getStaticConfigValue("USE_ICONV") && isset($this->output_charset) 
